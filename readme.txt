...
* Fix: Sermons now use the episode artwork you set in Planning Center. Planning Center returns artwork as a file object with named renditions, which the plugin did not read — so every synced sermon silently fell back to a still frame from its video. Sermons will pick up their real artwork on the next sync.

...
class VersionUpdate {
	...
	public function upgrade_to_cmb2() {
		...
		$cpc_main_options     = get_option( 'cpc_main_options', array() );
		$cpc_mp_options       = get_option( 'cpc_mp_options', array() );

		$cpc_main_options['chms'] = 'mp';
		...
		foreach ( $old_new_mapping as $old_key => $new_key ) {
			if ( isset( $mp_api_config[ $old_key ] ) && ! isset( $cpc_main_options[ $new_key ] ) ) {
				$cpc_main_options[ $new_key ] = $mp_api_config[ $old_key ];
			}
		}

		if ( isset( $group_mapping['fields'] ) && ! isset( $cpc_mp_options['group_fields'] ) ) {
			$cpc_mp_options['group_fields'] = wp_json_encode( array_values( $group_mapping['fields'] ) );
		}

		if ( ! empty( $custom_group_mapping ) ) {
			$cpc_mp_options['group_mapping'] = wp_json_encode( $custom_group_mapping );
		}
		...
		if ( isset( $group_mapping['mapping'] ) ) {
			foreach ( $group_mapping['mapping'] as $key => $value ) {
				...
				if ( ! isset( $cpc_mp_options[ $compound_key ] ) ) {
					$cpc_mp_options[ $compound_key ] = $value;
				}
			}
		}

		update_option( 'cpc_main_options', $cpc_main_options );
		update_option( 'cpc_mp_options', $cpc_mp_options );
